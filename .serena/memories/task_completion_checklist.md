...
38. cutover migration 러너를 건드렸다면 `member-rejects.json`, `timetable-rejects.json`, `minecraft-rejects.json`, `course-matches.json` 생성과 unknown timetable user reject 정책, live MySQL `courses` lookup 매칭 규칙을 함께 확인한다.
