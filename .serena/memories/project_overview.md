...
- support: 문의/신고(게시글/댓글/회원/채팅 메시지/일반 채팅방/택시파티)/앱 버전/법적 문서/학식. 학식은 기존 `menus`와 함께 `categories`, `menuEntries`(stable weekly id, title, badges, 실제 `likeCount`/`dislikeCount`, `myReaction`)를 제공하며 가격은 계약에서 제외한다. 보조 태그는 관리자 입력 메타데이터로 저장하고, 관리자 요청의 `likeCount`/`dislikeCount`는 deprecated 입력으로 남기되 저장 시 무시한다. 같은 주 안의 동일 `category + title`은 날짜가 달라도 동일 메타데이터를 강제한다. `menus` 검증 시 빈 카테고리 생략은 `menuEntries`의 빈 배열과 같은 의미로 본다. 사용자 반응은 `PUT /v1/cafeteria-menu-reactions/{menuId}` 한 개의 upsert API로 등록/전환/취소하며, source of truth는 `cafeteria_menu_reactions` row다.
