...
- `domain/image` + `infra/storage`: 이미지 업로드/썸네일 생성과 provider별 storage 추상화. `ProfileImageStorageService`가 내부 `PROFILE_IMAGE` URL만 식별해 원본 + `_thumb` 파일 정리를 담당하고, `StorageRepository.resolveRelativePath`를 `LocalStorageRepository`/`FirebaseStorageRepository`가 구현해 public URL을 storage key로 복원한다.
