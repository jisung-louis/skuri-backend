...
- `domain/notice/controller/NoticeController.java`, `MemberNoticeController.java`, `NoticeCommentController.java`, `domain/notice/service/NoticeService.java`: 학교 공지 조회/읽음/좋아요/북마크/댓글 API와 `PATCH /v1/notice-comments/{commentId}` content 수정 계약, `notice_bookmarks` 저장 모델, withdrawal cleanup 규칙을 관리
