...
- `domain/chat/service/ChatService.java`, `ChatMessageOrderGenerator.java`, `domain/chat/controller/ChatRoomController.java`, `domain/chat/dto/request/UpdateChatRoomReadRequest.java`: 공개방 visibility(joined/not joined summary, 학과방 노출 제한), 공개방 create/join/leave, active member 검증, 메시지/summary 실시간 fan-out, 학과방 membership 정리, 서버 저장 시 `messageOrder`를 부여하고 `getMessages`의 `createdAt DESC + messageOrder` 커서 tie-breaker 해석을 담당한다. 공개방 join/leave와 학과방 자동 leave는 실제 `SYSTEM` chat message를 저장하고 `/topic/chat/{chatRoomId}` + `/user/queue/chat-rooms`로 반영하며, join 직후 생성된 시스템 메시지 때문에 신규 멤버 unread가 1로 늘지 않도록 `lastReadAt`을 최신 메시지 시각으로 즉시 보정한다. `removeMembership(...)`는 summary event 중복을 막기 위해 summary publish 여부를 caller가 제어한다. `PATCH /v1/chat-rooms/{id}/read`는 메시지 `createdAt`처럼 timezone 없는 `LocalDateTime` 문자열과 ISO 8601 `Z`/offset 문자열을 모두 받고, timezone 없는 입력은 `Asia/Seoul` 기준으로 `Instant`로 변환해 서비스로 전달한다.
