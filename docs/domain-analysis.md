...
특이사항:
  ...
  - 관리자 회원 목록의 이름 컬럼은 `members.realname`, OS 컬럼은 최근 활성 FCM 토큰의 `fcm_tokens.platform`을 사용한다.
  - `currentAppVersion`은 member별 canonical 저장 source가 아직 없어 관리자 회원 목록 계약에 포함하지 않는다.
