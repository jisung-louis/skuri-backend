...
- Phase 11 구현 결정:
  ...
  - Member 관리자 권한 변경(`PATCH /v1/admin/members/{memberId}/admin-role`)은 `members.is_admin` boolean만 갱신하고 감사 로그를 남긴다. self-demotion/마지막 관리자 보호는 후속 정책 결정 전까지 런타임 제약을 추가하지 않는다.
...
> - 2026-03-29: Member Admin API P1 반영 — 관리자 회원 목록/상세/권한 변경 API를 Phase 1/Phase 11 규약에 동기화하고, `joinedAt,DESC` 목록 정렬 및 감사 로그 범위를 문서화
