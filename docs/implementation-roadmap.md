...
- 기본 storage provider는 **LOCAL 파일시스템**이며, `StorageRepository` 인터페이스를 통해 후속 cloud provider(S3/OCI/Firebase 등) 구현체를 교체 가능하게 둔다.
...
> - 2026-03-10: Phase 12 구현 반영 — `/v1/images` 런타임/OpenAPI/Postman, `StorageRepository` + 로컬 파일시스템 provider, context 권한 정책(`POST/CHAT/PROFILE` 인증, `APP_NOTICE` 관리자)을 문서 기준으로 동기화
