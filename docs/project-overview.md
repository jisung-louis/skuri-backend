...
│   ├── party/        택시 파티
│   ├── setting/      설정 관련
│   ├── storage/      서버 업로드 API(`/v1/images`) 연동 + 업로드 URL 관리
│   ├── timetable/    시간표 데이터
│   └── user/         사용자 프로필
├── contexts/         전역 상태
│   ├── AuthContext.tsx
...
- 클라이언트는 `react-native-image-picker`로 이미지를 선택
- 서버 업로드 API `POST /v1/images`로 multipart 업로드
- 서버가 원본/썸네일 URL, width/height/size/mime 메타데이터를 계산해 응답
- Board/Chat/AppNotice/Profile은 업로드 API가 반환한 URL을 기존 입력 필드에 그대로 재사용
