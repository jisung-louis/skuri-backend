...
대학교 특성상 같은 방향으로 이동하는 학생이 많지만 택시를 혼자 타는 경우가 많습니다. SKURI Taxi는 동승자를 빠르게 모집하고 채팅과 정산까지 앱 안에서 해결할 수 있게 합니다. 여기에 학교 공지, 커뮤니티 게시판, 시간표·학식 등의 생활 정보를 통합하여 성결대 학생의 캠퍼스 생활 플랫폼 역할을 합니다.
...
| 게시판 댓글 알림  | `boardCommentNotifications` | 내 글/댓글에 댓글/답글                                       |
...
| `scheduledRSSFetch` | 평일 8~20시, 10분마다 | 학교 공지 RSS 자동 크롤링 → Firestore `notices` 컬렉션에 신규/변경분 반영           |
