...
현재 기본 runtime provider는 **LOCAL 파일시스템**이며, 후속 S3/OCI/Firebase 구현체를 같은 인터페이스에 연결할 수 있습니다.
...
현재 provider는 LOCAL 파일시스템이며, 공개 URL prefix/base URL은 설정값으로 관리합니다.

| 현재 기본 provider | 후속 교체 대상 |
|--------------------|----------------|
| LOCAL 파일시스템 | AWS S3, OCI Object Storage, Firebase Storage, MinIO 등 |
