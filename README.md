...
4. 채팅: 공개 채팅방, 실시간 메시지
5. 학교 생활 정보: 시간표, 학식, 학사 일정, 캠퍼스 배너
6. 마인크래프트: 계정 등록, 서버 상태/온라인 플레이어 SSE, 공개방 채팅 bridge
7. 운영 지원: 문의 접수, 신고 접수, 앱 버전 관리, 관리자 대시보드

대상 사용자: `@sungkyul.ac.kr` 계정을 가진 성결대학교 학생

## 3. 저장소 현재 상태(백엔드 관점)

현재 저장소는 **Phase 13(마인크래프트 Spring 전환)** 구현까지 반영된 상태입니다.
...
- Notice 도메인 (Phase 5)
  ...
  - `rssPreview` / `bodyHtml` / `bodyText` / `thumbnailUrl(TEXT 저장형 캐시)` 저장 구조 + `summary`(향후 AI 요약 예약)
  - `/v1/notices`는 목록 전용 projection으로 필요한 컬럼만 select하고, 저장된 `thumbnailUrl`을 사용해 응답 생성
  - 기존 공지의 `thumbnailUrl`은 `migration.plan=NOTICE_THUMBNAILS` backfill로 DB `body_html`만 읽어 보정
  - `thumbnailUrl`은 `http/https` 또는 기존 상대경로 이미지 위주로만 저장하고, `data:`/`blob:`/과도하게 긴 값은 `null` 처리
...
- Support 도메인 (Phase 7)
  ...
  - 학식 응답은 기존 `menus`와 함께 `categories`, `menuEntries`(보조 태그, 실제 좋아요/싫어요 집계, `myReaction`)를 제공
  - 학식 반응 API `PUT /v1/cafeteria-menu-reactions/{menuId}` 제공
  - 같은 주의 같은 카테고리/메뉴명은 날짜가 달라도 동일 메타데이터를 강제하고, category 코드는 영문/숫자/_/-만 허용한다
  - 메뉴 ID는 `weekId + category + title` 기준의 opaque stable id이며, 반응 upsert는 주차 단위 직렬화로 재시도 안전성을 보장한다
  - 관리자 `likeCount`/`dislikeCount` 입력값은 deprecated이며 저장 시 무시, 실제 응답 카운트는 사용자 반응 row가 source of truth
...
- 이미지 업로드 인프라 (Phase 12)
  - 공통 업로드 API `POST /v1/images`
  - `StorageRepository` 추상화 + `LOCAL`/`FIREBASE` provider 지원
  - context별 정책: `POST_IMAGE`, `CHAT_IMAGE`, `PROFILE_IMAGE`는 인증 사용자, `APP_NOTICE_IMAGE`는 관리자 전용
  - Board/Chat/AppNotice/Profile의 기존 URL 직접 입력 계약은 유지하되, `/v1/images` 응답을 그대로 재사용 가능
- 마인크래프트 도메인 (Phase 13)
  - `GET /v1/minecraft/overview`, `GET /v1/minecraft/players`
  - `GET/POST/DELETE /v1/members/me/minecraft-accounts`
  - `GET /v1/sse/minecraft`, `GET /internal/minecraft/stream`
  - 플러그인 bridge API: `/internal/minecraft/chat/messages`, `/internal/minecraft/server-state`, `/internal/minecraft/online-players`
  - `public:game:minecraft` 공개방과 앱 STOMP, 플러그인 SSE outbox 연동
  - IMAGE 메시지 outbound placeholder text, Minotar avatar, shared secret 인증 반영
- CI
...
- 이미지 업로드는 기본적으로 `MEDIA_STORAGE_PROVIDER=LOCAL`일 때 `var/media`(호스트 실행) 또는 `/app/var/media`(Docker) 경로를 사용합니다. `MEDIA_STORAGE_PROVIDER=FIREBASE`로 바꾸면 `MEDIA_STORAGE_FIREBASE_BUCKET` 기준 Firebase Storage를 사용하고, 업로드 응답은 tokenized download URL을 반환합니다.
- 로컬 LOCAL provider에서는 `MEDIA_STORAGE_PUBLIC_BASE_URL`을 비워 두면 `server.port + MEDIA_STORAGE_URL_PREFIX` 기준 기본 공개 URL을 사용합니다.
...
- dry-run/apply 모두 네트워크 재크롤링 없이 `notices.body_html`만 읽고, keyset(`id > lastId`) batch로 `thumbnail_url`을 갱신한다.
- `thumbnail_url`는 `TEXT` 컬럼이며, backfill/sync는 `data:`/`blob:`/과도하게 긴 `img[src]`를 저장하지 않고 `null` 처리한다.
- summary report에는 `scanned`, `updated`, `no_image`, `failed` 집계가 기록된다.
